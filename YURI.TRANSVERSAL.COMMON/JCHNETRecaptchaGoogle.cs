...
{
    public static class JCHNETRecaptchaGoogle
    {
        public static bool Validar(ref JCHNETReCaptchaGoogleDto post, ref string mensaje)
        {
            ...
            {
                post.EncodedResponse = JCHNETConversions.NothingToString(post.EncodedResponse);
                if ((JCHNETConversions.NothingToString(post.ValorReCaptcha) != post.EncodedResponse.Length.ToString()) || post.EncodedResponse.Length == 0)
                {
        ...
        private static bool ReCaptchaGoogleReply(JCHNETReCaptchaGoogleDto post, ref string mensaje)
        {
            try
            {
                ...
                if (post.ReCaptchaClaveSitioWeb == null | post.ReCaptchaClaveComGoogle == null)
                    return true;
                if (JCHNETConversions.NothingToString(post.EncodedResponse) == "")
                {
