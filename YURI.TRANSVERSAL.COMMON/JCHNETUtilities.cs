...
namespace YURI.TRANSVERSAL.COMMON
{
    ...
    public static class JCHNETUtilities
    {
        ...
        public static bool ValidarCertificadoDigital(byte[] byteCertificado, string claveCertificado, ref string mensaje)
        {
            try
            {
                using (X509Certificate2 cer = new X509Certificate2(byteCertificado, claveCertificado))
                {
                    ...
                    if (ExpirationDate < DateTime.Now) throw new Exception("Certificado digital caducado");
                };
                return true;
            }
            catch (JCHNETException ex)
            {
        ...
        public static string CadenaConexion(string dataSource, string initialCatalog, string userId, string password, string cryptoKey, ref string mensaje, long TimeOut = 120)
        {
            try
            {
                if (string.IsNullOrEmpty(cryptoKey)) throw new Exception("cryptoKey is null");
                if (string.IsNullOrEmpty(dataSource)) throw new Exception("dataSource is null");
                if (string.IsNullOrEmpty(initialCatalog)) throw new Exception("initialCatalog is null");
                ...
                password =  JCHNETCyrpto.DescifrarClave(password, cryptoKey);
                if (string.IsNullOrEmpty(password)) throw new Exception("password encrypt is invalid");
        ...
        public static string CadenaConexion(string prefijo, string cryptoKey, ref string mensaje, long TimeOut = 120)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(dataSource)) throw new Exception($"Config: {campo} is null");
                campo = $"{prefijo}.initialCatalog"; var initialCatalog = System.Configuration.ConfigurationManager.AppSettings[campo];
                if (string.IsNullOrEmpty(initialCatalog)) throw new Exception($"Config: {campo} is null");
                campo = $"{prefijo}.userId"; var userId = System.Configuration.ConfigurationManager.AppSettings[campo];
                if (string.IsNullOrEmpty(userId)) throw new Exception($"Config: {campo} is null");
                campo = $"{prefijo}.password"; var password = System.Configuration.ConfigurationManager.AppSettings[campo];
                if (string.IsNullOrEmpty(password)) throw new Exception($"Config: {campo} is null");
                password = JCHNETCyrpto.DescifrarClave(password, cryptoKey);
                if (string.IsNullOrEmpty(password)) throw new Exception($"Config: {campo} encrypt is invalid");
        ...
        public static bool ValidarFirmaXmlArchivo(XmlDocument XmlArchivoFirmado, ref string mensaje)
        {
            try
            {
                ...
                if (nodeList.Count == 0) throw new Exception("Firma digital no encontrada en el archivo xml");
                if (certificates.Count == 0) throw new Exception("Certificado digital no encontrado en el archivo xml");
