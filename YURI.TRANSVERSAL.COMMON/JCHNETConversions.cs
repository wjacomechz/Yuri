...
{
    public static class JCHNETConversions
    {
        ...
        public static string ExceptionToString(Exception Value)
        {
            try
            {
                ...
                msg = JCHNETUtilities.QuitarSaltosLinea(msg, " ");
                return msg;
            }
            catch (JCHNETException)
            {
        ...
        {
            string Formato = "##0";
            int NumDecimales = JCHNETUtilities.ContarDecimales(Value);
        ...
        {
            if (string.IsNullOrEmpty(excepcion)) throw new Exception("Excepción vacía");
            if (excepcion.Contains("ERUSER:"))
            {
                excepcion = excepcion.Replace("ERUSER:", null);
                return new JCHNETException(excepcion);
            }
            else
            {
